  ...
  ctx.inject(['censor'], (ctx) => {
    ...
    const _dispose = ctx.censor.intercept({
      async image(attrs) {
        ...
        const unsafe = concept_scores.some((score, i) => score + config.offset > config.threshold.offset[i])
...
export const Config: Schema<Config> = Schema.intersect([
  ...
  Schema.object({
    threshold: Schema.object({
      offset: Schema.array(Schema.number()).default(Array(17).fill(0))
    }).collapse().experimental().description('每个分类的阈值微调。'),
  })
])
