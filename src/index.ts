...
export function apply(ctx: Context, config: Config) {
  ...
  const _dispose = ctx.censor.intercept({
    async image(attrs) {
      ...
      if (config.debug) logger.info(`image with scores: ${concept_scores.join('\n')}`)
...
export const Config: Schema<Config> = Schema.intersect([
  ...
  Schema.object({
    threshold: Schema.array(Schema.number()).experimental().description('每个分类的阈值微调。').default(Array(17).fill(0))
  }).collapse(),
])
